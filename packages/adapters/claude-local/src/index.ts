...
- outputInactivityTimeoutMs (number | null, optional): output-inactivity monitor around the claude child. Resets on every line of stdout that parses as JSON (claude runs with --output-format stream-json, so every system/assistant/user/result event — including the assistant turns that carry thinking output — counts). Defaults to 15 * 60_000 ms when unset or non-positive. Set to \`null\` to disable the monitor entirely (only do this for known-slow tasks; the platform-level silent-run safety net still applies). On fire, the adapter sends SIGTERM to the process group, waits 5s, then SIGKILL, and surfaces the run as failed with errorMessage "monitor: no claude output for {N}m {S}s" (errorCode claude_output_inactivity_monitor) so the run is retried, not left holding its slot. On the ACP lane the same knob arms the equivalent watchdog one layer up — it bounds silence in the ACP *event* stream (text/thought deltas, tool calls and tool-call updates, status and usage updates all reset it), defaults to 45 * 60_000 ms there because ACP events are coarser than stream-json lines, and cancels the turn with errorCode acpx_event_inactivity. That 45m window only covers silence *after* the turn's first event; silence before it is bounded by a separate, much shorter window (10 * 60_000 ms by default, capped at this knob's value so a shorter setting still wins), because a healthy turn emits its first event within seconds. Both windows share the same errorCode — classification and continuation retries are unchanged, only the errorMessage tells them apart — and \`null\` disables both. The same knob also bounds the session handshake that runs before any turn (spawn + initialize + session/new, 3 * 60_000 ms by default, capped by this knob and disabled with it), because the ACP runtime leaves spawn and initialize unbounded on this lane; that one fires with errorCode acpx_handshake_timeout. Without it an ACP turn on a local target has no bound at all: timeoutSec resolves to "unlimited" for local/SSH.
